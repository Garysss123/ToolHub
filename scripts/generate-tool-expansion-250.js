...
function jsonLd(tool){return JSON.stringify({'@context':'https://schema.org','@graph':[
...
<body class="font-sans" data-tool="${tool.slug}" data-tool-visual data-expansion-tool data-expansion-${batch} data-palette="${tool.palette}" data-layout="${tool.layout}" style="--tool-visual-accent:var(--tool-accent);--tool-visual-accent-rgb:var(--tool-accent-rgb)">
  ...
  <script src="/components/tool-expansion-${batch}.js?v=20260821-${batch}-1"></script>
...
function sidebarGroup(category,group){return `    <!-- ${marker} START Sidebar-${category} -->
    <div class="nav-group mb-2"><button class="w-full flex items-center justify-between px-6 py-2 text-slate-500 hover:text-slate-300 transition-colors group toggle-btn"><span class="text-xs font-semibold uppercase tracking-wider">${category}</span><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-300 chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg></button><div class="group-content overflow-hidden transition-all duration-300 max-h-0"><div class="px-2 py-1 space-y-0.5">
...
if(conflicts.length)throw new Error(`工具網址已存在：${conflicts.map(tool=>tool.slug).join(', ')}`);
const fileConflicts=tools.filter(tool=>{const file=path.join(root,tool.slug,'index.html');return fs.existsSync(file)&&!fs.readFileSync(file,'utf8').includes(`data-expansion-${batch}`)});
if(fileConflicts.length)throw new Error(`工具頁目錄已被其他頁面使用：${fileConflicts.map(tool=>tool.slug).join(', ')}`);
const sidebarPreflight=removeBlocks(fs.readFileSync(path.join(root,'components/sidebar.html'),'utf8'));
for(const [category,config] of Object.entries(categories)){
  if(!config.existing)continue;
  const heading=`>${category}</h2>`,start=home.indexOf(heading);
  if(start<0)throw new Error(`首頁找不到分類：${category}`);
  const sectionEnd=home.indexOf('</section>',start);
  if(home.lastIndexOf(`<button onclick="scrollContainer('${config.slider}', 1)"`,sectionEnd)<start)throw new Error(`首頁找不到 Slider：${config.slider}`);
  const sidebarHeading=`>${category}</span>`,sidebarStart=sidebarPreflight.indexOf(sidebarHeading);
  if(sidebarStart<0||sidebarPreflight.indexOf('<div class="px-2 py-1 space-y-0.5">',sidebarStart)<sidebarStart)throw new Error(`Sidebar 找不到分類：${category}`);
}
if(!home.includes('<!-- 分類五：SEO 與網站優化 -->'))throw new Error('首頁找不到 SEO 分類插入點');
if(!sidebarPreflight.includes('<!-- 群組 5: SEO 與網站優化 -->'))throw new Error('Sidebar 找不到 SEO 分類插入點');
if(!/<\/urlset>/.test(fs.readFileSync(path.join(root,'sitemap.xml'),'utf8')))throw new Error('Sitemap 缺少 urlset 結尾');
...
for(const [category,config] of Object.entries(categories)){
  ...
  const listStart=sidebar.indexOf('<div class="px-2 py-1 space-y-0.5">',start),close=sidebar.indexOf('</div>',listStart);if(listStart<start||close<listStart)throw new Error(`Sidebar 找不到分類內容：${category}`);
...
const urls=tools.map(tool=>`  <url><loc>https://toolhuben.com/${tool.slug}/</loc><lastmod>${lastmod}</lastmod><changefreq>monthly</changefreq><priority>0.8</priority></url>`).join('\n');
