...
async function main(){
  ...
  try{
    ...
    await c.call('Emulation.setDeviceMetricsOverride',{width:1280,height:900,deviceScaleFactor:1,mobile:false});await wait(180);
    await evaluate(`document.querySelector('footer [data-language-switch]').click()`);await waitFor(`location.pathname==='/ja/'&&!!document.querySelector('footer [data-language-switch]')`);check(await evaluate(`localStorage.getItem('toolhub-language')==='ja'`),'切到日文後未記住選擇');
    await evaluate(`document.querySelector('footer [data-language-switch]').click()`);await waitFor(`location.pathname==='/'&&!!document.querySelector('footer [data-language-switch]')`);check(await evaluate(`localStorage.getItem('toolhub-language')==='zh-Hant'`),'切回中文後未記住選擇');
