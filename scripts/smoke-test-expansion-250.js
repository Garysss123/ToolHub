...
const batch=process.env.TOOLHUB_SMOKE_BATCH||'250';
const {tools}=require(`./tool-expansion-${batch}-data`);
const locale=process.env.TOOLHUB_SMOKE_LOCALE==='en'?'en':'zh',urlPrefix=locale==='en'?'/en':'';
const root=path.resolve(__dirname,'..'),chrome=process.env.TOOLHUB_CHROME||'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',port=Number(process.env.TOOLHUB_SMOKE_PORT||5000+Number(batch)+(locale==='en'?1:0));
...
function dump(url,profile,size,screenshot){return new Promise((resolve,reject)=>{
  const args=['--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--disable-background-networking',`--user-data-dir=${profile}`,`--window-size=${size}`,'--virtual-time-budget=2600'];
  ...
  const timer=setTimeout(()=>{child.kill();reject(new Error('Chrome 檢查逾時'))},18000);
...
async function main(){
  ...
  await new Promise(resolve=>server.listen(port,'127.0.0.1',resolve));const profile=fs.mkdtempSync(path.join(os.tmpdir(),`toolhub-${batch}-smoke-`)),failures=[];
  try{
    for(let index=0;index<slugs.length;index++){
      ...
      try{
        const dom=await dump(`http://127.0.0.1:${port}${urlPrefix}/${slug}/`,profile,size,screenshot),reasons=[];
        if(!dom.includes(`data-expansion-${batch}`))reasons.push(`未接入 ${batch} 工具層`);
        ...
        if(!/<h1[\s>]/.test(dom)||!dom.includes(locale==='en'?'Frequently Asked Questions':'常見問題'))reasons.push('Hero 或 FAQ 缺失');
        if(locale==='en'&&!/<html lang="en"/.test(dom))reasons.push('英文頁面語系標記錯誤');
  ...
  console.log(`EXPANSION_${batch}_PAGES=${slugs.length}`);console.log(`EXPANSION_${batch}_FAILURES=${failures.length}`);if(visualDir)console.log(`EXPANSION_${batch}_VISUAL_DIR=${visualDir}`);
