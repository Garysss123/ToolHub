(function(){
  ...
  const copy={
    'zh-Hant':{switchLabel:'語言',switchTitle:'選擇語言',title:'選擇您的語言',body:'我們依照瀏覽器語系為您建議顯示語言。您之後可隨時切換。',recommended:'建議',zh:'繁體中文',zhNote:'台灣與香港使用者',en:'English',enNote:'English',ja:'日本語',jaNote:'日本語',later:'稍後再說'},
    en:{switchLabel:'Language',switchTitle:'Choose language',title:'Choose your language',body:'We recommend a language based on your browser settings. You can change it anytime.',recommended:'Recommended',zh:'Traditional Chinese',zhNote:'Taiwan and Hong Kong',en:'English',enNote:'English',ja:'Japanese',jaNote:'日本語',later:'Maybe later'},
    ja:{switchLabel:'言語',switchTitle:'言語を選択',title:'言語を選択',body:'ブラウザの言語設定に合わせて表示言語をおすすめします。言語はいつでも切り替えられます。',recommended:'おすすめ',zh:'繁體中文',zhNote:'台湾・香港',en:'English',enNote:'英語',ja:'日本語',jaNote:'日本語',later:'後で'}
  }[current];
  ...
  function switchTo(locale){store(locale);const target=targetFor(locale);if(target!==location.pathname+location.hash)location.assign(target);else closePrompt()}
  ...
  function showPrompt(force=false){
    if((!force&&(preferred||promptDismissed()))||document.getElementById('language-welcome'))return;
  ...
  document.addEventListener('click',event=>{
    const switcher=event.target.closest('[data-language-switch]');if(switcher){event.preventDefault();showPrompt(true);return}
